...
export default function FeedList({ feeds, onFeedItemClick, title = "Feeds", emptyMessage = "No pings found nearby." }: FeedListProps) {
  ...
  return (
    <div className="h-full overflow-y-auto bg-black border-r border-gray-300">
      <h2 className="text-lg font-semibold p-4 border-b border-gray-300">{title}</h2>
      {feeds.length > 0 ? (
        <ul>
          {feeds.map((feed) => (
            <li
            ...
            >
              <div className="flex items-center mb-2">
                  ...
                  <span className="font-semibold text-sm">{feed.uid}</span>
              </div>
